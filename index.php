...
$text=chop(str_replace(array("goo.gl","twitter","google")," * ",str_replace("@","@ ",strip_tags($kd1[1][8]))));
$tweetid=$kd2[1][0];
...
if ($tweetid > $check){
	...
	if (strlen($text)>0){
		...
		if($language != $la && strlen($la)>0){
		    $translate=json_decode(scurl('https://www.translate.com/translator/ajax_translate',1,'text_to_translate='.urlencode($text).'&source_lang='.$language.'&translated_lang='.$la.'&use_cache_only=false','','https://www.translate.com/'),1)["translated_text"];
		    $tr='翻译:'.chop(htmlentities(urlencode(str_replace(array("goo.gl","twitter","google","人们致以")," * ",$translate))))."\n";
		}else{
		    $tr='';
		}
