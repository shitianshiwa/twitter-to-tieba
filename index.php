...
$textold=str_replace("\n"," ",str_replace("&"," ",str_replace(">"," ",str_replace("<"," ",str_replace("'"," ",str_replace(""," ",$b["state"]["timeline"]["items"][0]["data"]["tweet"]["text"]["textParts"][0]["text"]))))));
...
$t = $from.':'.$textold.' '.$link.' '.$time;
if(strlen($t)>0){
$tr='text_to_translate='.$textold.'&source_lang=ja&translated_lang=zh&use_cache_only=false';
...
图片地址(如果有):'.$pic.'&_t=1484059982471&tag=11&upload_img_info=63d9f2d3572c11df37f90b6a6a2762d0f703c24b%2C164%2C59%2Cfalse&fid='.$fid.'&src=1&word='.$tbn.'&tbs='.$tbs.'&z='.$tid;
$ch = curl_init($url);
...
$log = fopen('log.txt',"a");
    flock($log,LOCK_EX);
    fwrite($log,'['.time().','.$content.','.$name.','.$data.','.$translate.']'."\r\n");
    fclose($log,LOCK_UN);
if($a['no']>"0") {
  echo "失败！";
}else {echo '发送成功';}
